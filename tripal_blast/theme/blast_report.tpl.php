...
if ($xml) {
  ...
  $header = array(
    ...
    'select-box' => array('data' => 'Pick one', 'class' => array('select-box'))
  );
    ...
    if($children_count != 0) {
      ...
      foreach($iteration->{'Iteration_hits'}->children() as $hit) {
        if (is_object($hit)) {
          ...
          $qseq = $hit->{'Hit_hsps'}->{'Hsp'}->{'Hsp_qseq'};
          $query_name = $iteration->{'Iteration_query-def'};

	  $qseq2 = arg(3);
	  $qname = arg(4);

          $family = $linkout_match[1];
          $countRows++;
          $ipLink = l(
            $qseq2,
            "/iplant/$family/$qseq2/$qname",
            array('attributes' => array('target' => '_blank'))
          );
          ...
          $row = array(
            'data' => array(
              ...
              'select-box' => array('data' => $ipLink, 'class' => array('select-box'))
            ),
